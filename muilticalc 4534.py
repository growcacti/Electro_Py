...
class FlexibleCalculator:
    ...
    def setup_ui(self):
        ...
        # Result Display
        ttk.Label(self.root, text="Result:").grid(row=4, column=0, padx=10, pady=5, sticky="e")
        self.result_var = tk.StringVar()
        ttk.Entry(self.root, textvariable=self.result_var, state="readonly", width=50).grid(row=4, column=1, columnspan=2, padx=10, pady=5)

        # Graphing Section
        ttk.Button(self.root, text="Plot Results", command=self.plot_results).grid(row=5, column=0, columnspan=3, pady=10)

        # Canvas for Graph
        self.canvas_frame = ttk.Frame(self.root)
        self.canvas_frame.grid(row=6, column=0, columnspan=4, pady=10)
