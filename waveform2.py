...
def makegraph(event,A,B,C,D):
    ...
    for x in range(0,760):
        ...
        xy.append(x)
        # y coordinates
        y1 = float(A*math.sin(((0.25*math.pi)* B)*(x+C)) + D)
        y2 = float(A*math.sin(((0.25*math.pi)* B)*(x+C)) ** 3)
        y3 = float(A*math.sin(((0.25*math.pi)* B)* (x+C)) ** 5)
        y4 = float(A*math.sin(((0.25*math.pi)* B)* (x+C)) ** 7)
...
A = Scale(root, variable=A, length=400, from_=1, to = 390)
A.place(x=100, y=440)
...
D = Scale(root, variable=D, length=400, from_=1, to = 400)
D.place(x=100, y=560)
