...
class TabFrame:
    def __init__(self):
        ...
        self.f1 = nb.add_tab("SPECIAL CUSTOM CALC")
        ttk.Label(self.f1, text="Iter Calc").grid(row=1,column=1)
        self.f2 = nb.add_tab("Voltage_Current_Resistance")
        ...
        self.f5 = nb.add_tab("Standard Calculator")
        ...
        self.f7 = nb.add_tab("Percentage Calc")
        ttk.Label(self.f7, text="Percentage +/- Spec Calc").grid(row=1,column=1)
        self.f8 = nb.add_tab("Time Duration")
        ttk.Label(self.f8, text="Time Duration").grid(row=1,column=1)
        self.f9 = nb.add_tab("Text Viewer")
        ttk.Label(self.f9, text="Quickly look at tedxt files").grid(row=1,column=1)
        self.f13 = nb.add_tab("History Store")
        ttk.Label(self.f13, text="History Input / Output").grid(row=1,column=1)
        self.f14 = nb.add_tab("History Read")
        ttk.Label(self.f14, text="Load Data File").grid(row=1,column=1)
        self.f22 = nb.add_tab("Convert Units")
        ttk.Label(self.f22, text="Common Unit Conversions").grid(row=1,column=1)
...
class Mem:
    def __init__(self):
        self.lb = tk.Listbox(tab.f13,width=80, height=45)
        self.lb.grid(row=1, column=1, rowspan=4,columnspan=5)
        self.path = os.getcwd()
        self.btn_1 = tk.Button(tab.f14, text="Open", bd=3,command=self.open_file)
        self.btn_1.grid(row=1, column=0)
        self.btn_2 = tk.Button(tab.f14,text="Save",bd=3,command=self.save_file)
        self.btn_2.grid(row=2, column=0)
        self.btn_3 = tk.Button(tab.f14, text="Clear",bd=3, command=self.clear)
        self.btn_3.grid(row=3, column=0, sticky="ew", padx=5, pady=5)
        self.textwidget = ScrolledText(tab.f14, bg="white", bd=12, height=35,width=60,)
        self.textwidget.grid(row=0, column=2,sticky="w")
...
class ReverseParalellResistanceCalculator:
    ...
    def on_submit(self):
        self.series = self.series_var.get()
        self.tolerance = self.tolerance_var.get()
        
        # Removed the call to the non-existent method
        # self.standard_values = self.get_base_standard_values(series)
        ...
        for combination in self.generate_combinations(self.desired_resistance, self.max_num_resistors):
        ...
        self.text_area.insert(tk.END, f"{resistors} -> {res:.2f}Ω (Error: {err*100:.2f}%)\n")
        mem.sto(tk.END, f"{resistors} -> {res:.2f}Ω (Error: {err*100:.2f}%)\n")
...
##                print(f"Testing combination {combination}: {combined_resistance}Ω, Error: {error}")  # Debug print
            ...
            if found_combinations:  # Corrected here
                break
...
class DurationCalculatorApp:
    def __init__(self, parent):
        self.parent = parent
        self.setup_ui()

    def setup_ui(self):
        # Create labels for date fields
        tk.Label(self.parent, text="Start Date:").grid(row=0, column=0, padx=10, pady=10)
        tk.Label(self.parent, text="End Date:").grid(row=0, column=4, padx=10, pady=10)

        self.start_cal = Calendar(self.parent, selectmode='day', date_pattern='y-mm-dd')
        self.start_cal.grid(row=1, column=0, padx=10, pady=10)

        self.start_year, self.start_month, self.start_day = tk.IntVar(), tk.IntVar(), tk.IntVar()
        self.setup_date_entry_fields(self.start_year, self.start_month, self.start_day, row_start=2, col_start=0)

        self.end_cal = Calendar(self.parent, selectmode='day', date_pattern='y-mm-dd')
        self.end_cal.grid(row=1, column=5, padx=10, pady=10)

        self.end_year, self.end_month, self.end_day = tk.IntVar(), tk.IntVar(), tk.IntVar()
        self.setup_date_entry_fields(self.end_year, self.end_month, self.end_day, row_start=2, col_start=6)

        # Create labels for time fields
        tk.Label(self.parent, text="Start Time (HH:MM):").grid(row=2, column=0, padx=10, pady=10)
        tk.Label(self.parent, text="End Time (HH:MM):").grid(row=2, column=5, padx=10, pady=10)

        self.start_hour, self.start_minute = Combobox(self.parent, values=list(range(24))), Combobox(self.parent, values=list(range(60)))
        self.end_hour, self.end_minute = Combobox(self.parent, values=list(range(24))), Combobox(self.parent, values=list(range(60)))

        self.setup_comboboxes(self.start_hour, self.start_minute, row_start=10, col_start=0)
        self.setup_comboboxes(self.end_hour, self.end_minute, row_start=10, col_start=6)

        tk.Button(self.parent, text='Calculate Duration', command=self.calculate_duration).grid(row=22, column=3)
        self.result_text = tk.Text(self.parent, height=10, width=70)
        self.result_text.grid(row=25, column=0, columnspan=3)

        self.start_cal.bind("<<CalendarSelected>>", lambda _: self.update_entry_fields(self.start_cal, self.start_year, self.start_month, self.start_day))
        self.end_cal.bind("<<CalendarSelected>>", lambda _: self.update_entry_fields(self.end_cal, self.end_year, self.end_month, self.end_day))

    def setup_date_entry_fields(self, year_var, month_var, day_var, row_start, col_start):
        date_entries = [year_var, month_var, day_var]
        labels = ["Year:", "Month:", "Day:"]
        for i, (var, label) in enumerate(zip(date_entries, labels)):
            tk.Label(self.parent, text=label).grid(row=row_start + i, column=col_start)
            tk.Entry(self.parent, textvariable=var).grid(row=row_start + i, column=col_start + 1)

    def setup_comboboxes(self, hour_var, minute_var, row_start, col_start):
        labels = ["Hour:", "Minute:"]
        variables = [hour_var, minute_var]
        for i, (var, label) in enumerate(zip(variables, labels)):
            tk.Label(self.parent, text=label).grid(row=row_start + i, column=col_start)
            var.set("5")  # Set initial value
            var.grid(row=row_start + i, column=col_start + 1)

    def update_entry_fields(self, date, year_var, month_var, day_var):
        date_str = date.get_date()
        date_value = datetime.strptime(date_str, "%Y-%m-%d")
        year_var.set(date_value.year)
        month_var.set(date_value.month)
        day_var.set(date_value.day)

    def calculate_duration(self):
        start_year_val = self.start_year.get()
        start_month_val = self.start_month.get()
        start_day_val = self.start_day.get()
        start_hour_val = int(self.start_hour.get())
        start_minute_val = int(self.start_minute.get())

        end_year_val = self.end_year.get()
        end_month_val = self.end_month.get()
        end_day_val = self.end_day.get()
        end_hour_val = int(self.end_hour.get())
        end_minute_val = int(self.end_minute.get())

        try:
            start_date = datetime(
                start_year_val, start_month_val, start_day_val,
                start_hour_val, start_minute_val
            )
            end_date = datetime(
                end_year_val, end_month_val, end_day_val,
                end_hour_val, end_minute_val
            )

            delta = relativedelta(end_date, start_date)

            result = f"Duration: {delta.years} years, {delta.months} months, {delta.days} days, "
            result += f"{delta.hours} hours, {delta.minutes} minutes\n"
            self.result_text.insert(tk.END, result)
        except ValueError:
            self.result_text.insert(tk.END, "Invalid date/time values.\n")


duration_calculator = DurationCalculatorApp(tab.f8)
...
unit_convert = Weight_Length_Liquid(tab.f22)
...
class LbTx(tk.Frame):
    def __init__(self, parent):
        ...
        self.tx2 = ScrolledText(self.f2, height=45, width=60)

        self.tx2.grid(row=3, column=15)

        self.tx2.insert("1.0", tk.END)
        ...
        self.btn_open = tk.Button(
            ...
            text="Open",
            bd=1,
            command=lambda: self.open(self.tx2),
        )
        ...
        self.btn_save = tk.Button(
            ...
            text="Save As...",
            bd=2,
            command=lambda: self.save(self.tx2),
        )
        self.btn_save.grid(row=2, column=0)
        self.btn_clear = tk.Button(
            self.f2, text="Clear", bd=1, command=lambda: self.clear(self.tx2)
        )
        self.btn_clear.grid(row=3, column=0, sticky="ew")
        self.btn_grab = tk.Button(
            self.f2, text="Grab", bd=1, command=self.ggtext())
    ...
    def ggtext(self):
        grab_txt = self.tx.get("1.0", tk.END)
        self.tx2.delete("1.0", tk.END)
        self.tx2.insert(tk.END, grab_txt)
    ...
    def listing(self, event=None):
        ...
        # The fix: Check if 'file' is a directory before opening
        if os.path.isdir(file):
            print(f"'{file}' is a directory, cannot open as a file.")
            return
        else:
