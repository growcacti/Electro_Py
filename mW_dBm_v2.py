...
if len(sys.argv) > 2:
    ...
    if len(sys.argv) > 1:
        if "MW" in sys.argv[1].upper():
            ...
            dBm = mW2dBm(mW)
            ff = tk.Tk()
            l = tk.Label(ff, text= "%d mW = %0.2f dBm" %(mW, dBm))
            l.pack()
            #print( "%d mW = %0.2f dBm" %(mW, dBm))
             ...
             if "-" in sys.argv[1]:
                ...
                mW = dBm2mW(dBm)
                l2 = tk.Label(ff, "%0.2f dBm = %0.10f mW" %(dBm, mW))
                l2.pack()
...
label1 = Label(root, text='mW to dBm')
label1.grid(row=0, column=0)
 
entry1 = Entry(root, textvar=dBm, justify=RIGHT)
entry1.grid(row=0, column=1)
  
label2 = Label(root, text='dBm to mW')
label2.grid(row=1, column=0)
 
entry2 = Entry(root, textvar=mW, justify=RIGHT)
entry2.grid(row=1, column=1)
    
btn1 = tk.Button(root, text='mW to dBm', command=mW2dBm(mW.get()))
btn1.grid(row=3, column=2)
btn2 = tk.Button(root, text='dBm to mW', command=dBm2mW(dBm.get()))
